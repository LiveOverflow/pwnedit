...
def run_sudoedit(arg, env):
    ...
    # execute it
    p = subprocess.Popen(_cmd, env=env, bufsize=0, 
    ...
    if os.path.isfile('/tmp/heap'):
        ...
        _backtrace_short = ' '.join([i.split()[3] for i in backtrace if len(i.split())>1 and i.split()[1].startswith('0x')])
